...
hid_units = [64,32,32,16,16,16,16,8] # numbers of hidden units per each attention head in each layer
n_heads = [1, 1, 1,1,1,1,1,1] # additional entry for the output layer
residual = True
...
for train_ind, test_ind in kf.split(fmri_signals):
    ...
    if dataset != 'HCP' or dataset != 'ADHD':
    ...
    with tf.Graph().as_default():
        ...
        logits, _, predict= model.inference(nb_classes, fmri_net, is_train,
                                                        net_mat=bias_in,hid_units=hid_units,
        ...
        loss = model.BNCNN_loss(logits, lbl_in)

        accuracy = model.BNCNN_accuracy(logits, lbl_in)

        train_op = model.BNCNN_training(loss, lr)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(nb_epochs):
                ...
                while tr_step * batch_size < tr_size:
                    _, loss_value_tr, acc_tr, prediction = sess.run([train_op, loss, accuracy, predict],
                    ...
                    # print('Logit value is {}, ------------------------------'.format(prediction))
...
###---------------------------------------------------------------------------------------------
                ...
                if val_acc_avg/vl_step >= vacc_mx or val_loss_avg/vl_step <= vlss_mn:
                    ...
                    curr_step += 1
                    if curr_step == patience and train_acc_avg/tr_step>0.8:
            ...
            while ts_step * batch_size < ts_size:
                loss_value_ts, acc_ts, predict_label, actual_label = sess.run([loss, accuracy, predict, lbl_in],
                    feed_dict={
...
print('Final test loss:', test_loss_avg, '; Final test accuracy:', test_acc_avg)
    ...
    # if(test_acc_avg>0.69 and test_acc_avg<0.72):
    #         out_labels = np.asarray([np.concatenate(predict_labels, axis=0), np.concatenate(actual_labels, axis=0)])
    #         np.save('Data_BNF/'+dataset+'/result_BNCNN.npy', out_labels)
    #         break
