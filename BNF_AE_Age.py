...
from models import GAT_BNF, GAT_bi_BNF, GAT_AE_BNF
...
recon_lr_weight = 5
hid_units = [64,64,32] # numbers of hidden units per each attention head in each layer
n_heads = [18,12,9] # additional entry for the output layer
residual = True
...
fmri_signals, labels, graphs = \
    Data_processing.load_data('Data_BNF/'+dataset+'/','Data_BNF/'+dataset+'/labels_PicVocab.csv', augmentation)
...
while (True):
    ...
    for train_ind, test_ind in kf.split(fmri_signals):
        ...
        tr_labels = np.expand_dims(np.asarray(tr_labels), axis=1)
        val_labels = np.expand_dims(np.asarray(val_labels), axis=1)
        test_labels = np.expand_dims(np.asarray(test_labels), axis=1)
        ...
        with tf.Graph().as_default():
            with tf.name_scope('input'):
                ...
                lbl_in = tf.placeholder(dtype=tf.float32, shape=(None, nb_classes))
            ...
            logits, reconstruct_fmri, reconstruct_DTI, prediction = model.inference(ftr_in, nb_classes, fmri_net, nb_slot, is_train,
            ...
            loss = model.AE_regression_loss(logits, lbl_in, fmri_net, reconstruct_fmri, bias_in, reconstruct_DTI, recon_lr_weight)
    ...
    ###---------------------------------------------------------------------------------------------
                    ...
                    if val_acc_avg/vl_step >= vacc_mx or val_loss_avg/vl_step <= vlss_mn:
                        ...
                        curr_step += 1
                        if (curr_step == patience or train_acc_avg/tr_step>0.90) and epoch>50:
                ...
                while ts_step * batch_size < ts_size:
                    loss_value_ts, acc_ts, recon_net, original_net = sess.run([loss, accuracy,reconstruct_fmri,fmri_net],
                        feed_dict={
