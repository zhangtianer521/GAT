...
from models import GAT_BNF, GAT_bi_BNF, GAT_AE_BNF, GAT_FC_BNF, GAT_Node2Vec_BNF
...
n_heads = [6, 6, 6] # additional entry for the output layer
residual = True
nonlinearity = tf.nn.relu
model = GAT_Node2Vec_BNF
...
fmri_signals, labels, graphs = \
    Data_processing.load_data('Data_BNF/'+dataset+'/','Data_BNF/'+dataset+'/labels_gender.csv', augmentation)

fmri_signals = np.load('/home/local/ASUAD/wzhan139/Dropbox (ASU)/Project_Code/GAT/node2vec-master/embed.npy')
...
while(True):
    ...
    for train_ind, test_ind in kf.split(fmri_signals):
        ...
        with tf.Graph().as_default():
            with tf.name_scope('input'):
                ...
                fmri_net = tf.placeholder(dtype=tf.float32, shape=(None,nb_nodes,64))
            ...
            logits, _ = model.inference(ftr_in, nb_classes, fmri_net, nb_CAM, is_train,
            ...
            loss = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=lbl_in)
            loss = tf.reduce_mean(loss,axis=0)
            # loss = model.AE_classify_loss(logits, lbl_in, fmri_net, reconstruct_fmri, bias_in, reconstruct_DTI, recon_lr_weight)


            accuracy = model.accuracy(logits, lbl_in)
            f1, precision, recall = model.micro_f1(logits,lbl_in)
    ...
    ###---------------------------------------------------------------------------------------------
                    ...
                    if val_acc_avg/vl_step >= vacc_mx or val_loss_avg/vl_step <= vlss_mn:
                        ...
                        curr_step += 1
                        # if (curr_step == patience and train_acc_avg/tr_step>0.7) and epoch>10:
                        if (curr_step == patience ) and epoch > 10:
                ...
                while ts_step * batch_size < ts_size:
                    loss_value_ts, acc_ts, actual_label, f1_t, prec_t, recall_t = sess.run([loss, accuracy, lbl_in, f1, precision, recall],
                        feed_dict={
                ...
                test_f1_avg.append(ts_f1 / ts_step)
                test_prec_avg.append(ts_precision / ts_step)
                ...
                # group_cam = np.concatenate(group_cam,axis=0)
                # group_average_cam = np.mean(group_cam,axis=0)
                # batch_cam.append(group_cam[...,0])
                #####---------------------------------------------------



                # plt.matshow(original_net[5])
                # plt.matshow(recon_net[5])
                # plt.matshow(original_net[33])
                # plt.matshow(recon_net[33])
                # plt.show()
    ...
    test_f1_avg = np.asarray(test_f1_avg)
    test_f1_avg = np.mean(test_f1_avg)

    test_prec_avg = np.asarray(test_prec_avg)
    test_prec_avg = np.mean(test_prec_avg)

    print('--------------------------------------------------')
    print('Final test loss:', test_loss_avg, '; Final test accuracy:', test_acc_avg,
          '; Final test precision:', test_prec_avg, '; Final test f1:', test_f1_avg)

    # if(test_acc_avg>0.60):
    #     out_labels = np.asarray([np.concatenate(predict_labels, axis=0), np.concatenate(actual_labels, axis=0)])
    #     precision, recall, f1_score, _ = Data_processing.precision_recall_fscore_support(out_labels[0],out_labels[1])
    #     # print('precision is ', precision, '; recall is ', recall, '; f1-score is ', f1_score)
    #     # np.save('Data_BNF/'+dataset+'/result_cam.npy', out_labels)
    #     # np.save('Data_BNF/'+dataset+'/cam_map.npy', np.concatenate(batch_cam,axis=0))
    #     break
    break
